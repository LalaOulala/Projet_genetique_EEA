...
#define dim 128
#define tailleBinaireY0 15
#define tailleBinaireB 21
#define tailleBinaireDL 21
#define tailleBinaireLo 30
...
int main(void)
{
    ...
    for(i = 0; i < 100; i++)
    {
        ...
        ValeursY0[i] = genererValeurAleatoire(20000, 25000);
        ValeursB[i] = genererValeurAleatoire(1000000, 1500000);
        ValeursDeltaLambda[i] = genererValeurAleatoire(900000, 2000000);
        ValeursLambdaZero[i] = genererValeurAleatoire(655000000, 657000000);
    ...
    // Boucle principale pour générer de nouvelles générations jusqu'à atteindre le seuil de fitness ou la limite d'itérations
    do
    {
        ...
        iteration++;
    } while(enfants[indiceMeilleur].fitnessScore >= seuil && iteration != limite);
...
/*
on fixe le seuil de mutationGenetiqueAleatoire à 1%.                                                      //deja optimiser 
*/
individu mutationGenetiqueAleatoire(individu x) {
...
individu croisementGenetique(individu parent1 , individu parent2)
{
    ...
    r1 = rand() % tailleBinaireY0;  
    lim1 = rand() % tailleBinaireY0;
    if(r1 > lim1)
    {
        for(i = lim1; i <= r1; i++)
        {
    ...
    r2 = rand() % tailleBinaireB;
    lim2 = rand() % tailleBinaireB;
    if(r2 > lim2)
    {
        for(i = lim2; i <= r2; i++)
        {
    ...
    r3 = rand() % tailleBinaireDL;
    lim3 = rand() % tailleBinaireDL;
    if(r3 > lim3)
    {
        for(i = lim3; i <= r3; i++)
        {
    ...
    r4 = rand() % tailleBinaireLo;
    lim4 = rand() % tailleBinaireLo;
    if(r4 > lim4)
    {
        for(i = lim4; i <= r4; i++)
        {
