...
void ecriture(char nom_du_fichier[], double X_f, double Y_f); 
...
void calculs_nv_pts(char nom_du_fichier[], double Y0, double B, double DL, double Lo, datas x) ;
...
int main(void)
{
	...
	***************************************************************************************************/
	...
	do
	{
		...
		//on ecrase les individus contenu dans la premiere population et on stoke les nouveaux individus à leurs places.  
		for (i=0 ; i<dim ; i++)
		{	for(n=0  ; n<bit_Y0 ; n++)
				population[i].G1[n] = enfants[i].G1[n] ;
				//printf("population[%d].G1[%d] = enfant[%d].G1[%d]\n(%d) = (%d)\n\n",i,n,i,n,(population[i].G1[n]),(enfants[i].G1[n]));
			for(n=0  ; n<bit_B ; n++)
...
/*
	...
	for (int n = 0; n < 1024; n++) {
		...
		double image_f = f(g1, g2, g3, g4, X[n]);
		ecriture("valeur.txt", X[n], image_f); 
    }
*/

	calculs_nv_pts("B_egal_cent.txt", g1, g2, g3, g4, X);
...
// Fonction retournant la valeur de J(L)
double f(double Y0, double B, double DL, double Lo, double L)
{
...
double decodage(individu x, datas X, datas Y) {
    ...
    for (int n = 0; n < 1024; n++) {
		...
		// calcul de l'image de chaque point
		double image_f = f(Y0, B, DL, Lo, X[n]);
		ecriture("valeur.txt", X[n], image_f); 
    }
...
void calculs_nv_pts(char nom_du_fichier[], double Y0, double B, double DL, double Lo, datas x)
{
	...
	{
		// calcul de l'image de chaque point
		double image_f = f(Y0, 100.f, DL, Lo, x[n]);
